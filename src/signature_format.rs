...
impl FrequencyPeak {
    ...
    pub fn get_frequency_hz(&mut self) -> PyResult<f32> {
        ...
        Ok(self.corrected_peak_frequency_bin as f32 * (self.sample_rate_hz as f32 / 2.0 / 1024.0 / 64.0))
    }

    pub fn get_amplitude_pcm(&mut self) -> PyResult<f32> {
        // Not sure about this calculation but gives small enough numbers
        Ok((((self.peak_magnitude as f32 - 6144.0) / 1477.3).exp() * ((1 << 17) as f32) / 2.0).sqrt() / 1024.0)
    }

    pub fn get_seconds(&mut self) -> PyResult<f32> {
        // Assume that new FFT bins are emitted every 128 samples, on a
        // standard 16 KHz sample rate basis.
        Ok((self.fft_pass_number as f32 * 128.0) / self.sample_rate_hz as f32)
    }
}
...
impl DecodedSignature {
    ...
    fn new(
        ...
        DecodedSignature {sample_rate_hz: sample_rate_hz, number_samples: number_samples, frequency_band_to_sound_peaks: frequency_band_to_sound_peaks}
    }

    #[staticmethod]
    pub fn decode_from_binary(data: &[u8]) -> PyResult<DecodedSignature> {
    ...
    #[staticmethod]
    pub fn decode_from_uri(uri: &str) -> PyResult<DecodedSignature> {
        assert!(uri.starts_with(DATA_URI_PREFIX));
        Ok(DecodedSignature::decode_from_binary(&base64::decode(&uri[DATA_URI_PREFIX.len()..]).unwrap())?)
    }

    pub fn encode_to_binary(&mut self) -> PyResult<Vec<u8>> {
    ...
    pub fn encode_to_uri(&mut self) -> PyResult<String> {
